...
int parseDataHeader(FILE *fp, DataHeader* pHeader)
{
    ...
    //fprintf(stderr, "Supported trace file version: %d, %d\n", pHeader->version, pHeader->recordSize);
...
int readDataRecord(FILE *dataFp, DataHeader* dataHeader,
        int *threadId, unsigned int *methodVal, uint64_t *elapsedTime, uint64_t *wallTime)
{
    int id;
    int bytesToRead;
...
void dumpTrace()
{
    ...
    printf("Trace (threadID action usecs usecs class.method signature):\n");

    while (1) {
        MethodEntry* method;
        int threadId;
        unsigned int methodVal;
        uint64_t elapsedTime, wallTime;
         ...
         */
        if (readDataRecord(dataFp, &dataHeader, &threadId, &methodVal, &elapsedTime, &wallTime))
            break;
...
DataKeys* parseDataKeys(TraceData* traceData, const char* traceFileName, uint64_t* threadTime)
{
    ...
    while (1) {
         ...
         * Extract values from file.
         */
        if (readDataRecord(dataFp, &dataHeader, &threadId, &methodVal, &currentTime, &wallTime))
            break;
