...
public final class ActivityThread {
    ...
    static final class Profiler {
        ...
        public void startProfiling(int samplingIntervalUs) {
            if (profileFd == null || profiling) {
                return;
            }
            try {
                Debug.startMethodTracing(profileFile, profileFd.getFileDescriptor(),
                        8 * 1024 * 1024, 0, true, samplingIntervalUs);
                profiling = true;
            } catch (RuntimeException e) {
                Slog.w(TAG, "Profiling failed on path " + profileFile);
                try {
    ...
    private class ApplicationThread extends ApplicationThreadNative {
        ...
        public void profilerControl(boolean start, String path, ParcelFileDescriptor fd,
            ...
            pcd.path = path;
            pcd.fd = fd;
            Log.d("David", "profilerControl " + (start ? 1 : 0) + " " + path + " " + profileType);
    ...
    final void handleProfilerControl(boolean start, ProfilerControlData pcd, int profileType) {
        if (start) {
            Log.d("David", "handleProfilerControl " + profileType); 
            try {
                switch (profileType) {
                    default:                        
                        ...
                        // David's hack to enable sampling mode
                        mProfiler.startProfiling(profileType); 
                        //mProfiler.startProfiling();
                        break;
                }
            } catch (RuntimeException e) {
